...
public class GroupRightBean implements Serializable {
    ...
    public List<GroupRight> getCurrentGroupRights(int aStoreId, long aUserDetailId) {
        ...
        ResultSet rs = null;
        List<GroupRight> NewCurrentGroupRights = new ArrayList<>();
        try (
                Connection conn = DBConnection.getMySQLConnection();
                PreparedStatement ps = conn.prepareStatement(sql);) {
    ...
    public String getFunctionByTransType_Dep(String aTransTypeName, String aTransReasonName) {
        String x = "";
        try {
    ...
    public String[] getFunctionArrayList() {
        List<TransactionReason> trs = new TransactionReasonBean().getTransactionReasons();
        int n = trs.size();
        FunctionArrayList = new String[n];
        int i = 0;
        while (i < n) {
            FunctionArrayList[i] = Integer.toString(trs.get(i).getTransactionReasonId());
            i = i + 1;
        }
...
//    }
    ...
    public String getFcnName_Old(int aTransTypeId, int aTransReasonId) {
        /*
         TTI	TTN                     TRI	TRN
    ...
    public void allowAll(String aAllowWhat, boolean aIsAllow) {
        int ListItemIndex = 0;
        int ListItemNo = 0;
        try {
            ListItemNo = this.GroupRightsForEdit.size();
        } catch (NullPointerException npe) {
            ListItemNo = 0;
        }

        while (ListItemIndex < ListItemNo) {
            if (aAllowWhat.equals("View")) {
                if (aIsAllow) {
                    this.GroupRightsForEdit.get(ListItemIndex).setAllowView("Yes");
                } else {
                    this.GroupRightsForEdit.get(ListItemIndex).setAllowView("No");
                }
            }
            if (aAllowWhat.equals("Add")) {
                if (aIsAllow) {
                    this.GroupRightsForEdit.get(ListItemIndex).setAllowAdd("Yes");
                } else {
                    this.GroupRightsForEdit.get(ListItemIndex).setAllowAdd("No");
                }
            }
            if (aAllowWhat.equals("Edit")) {
                if (aIsAllow) {
                    this.GroupRightsForEdit.get(ListItemIndex).setAllowEdit("Yes");
                } else {
                    this.GroupRightsForEdit.get(ListItemIndex).setAllowEdit("No");
                }
            }
            if (aAllowWhat.equals("Delete")) {
                if (aIsAllow) {
                    this.GroupRightsForEdit.get(ListItemIndex).setAllowDelete("Yes");
                } else {
                    this.GroupRightsForEdit.get(ListItemIndex).setAllowDelete("No");
                }
            }
            ListItemIndex = ListItemIndex + 1;
        }
    ...
    public void allowAll(GroupRight aGroupRight, boolean aIsAllow) {
        try {
            if (aIsAllow) {
                aGroupRight.setAllowAdd("Yes");
                aGroupRight.setAllowEdit("Yes");
                aGroupRight.setAllowView("Yes");
                aGroupRight.setAllowDelete("Yes");
            } else {
                aGroupRight.setAllowAdd("No");
                aGroupRight.setAllowEdit("No");
                aGroupRight.setAllowView("No");
                aGroupRight.setAllowDelete("No");
            }
        } catch (NullPointerException npe) {
    ...
    public void allowAll(GroupRight aGroupRight) {
        try {
            if (aGroupRight.isAllowAll()) {
                aGroupRight.setAllowAdd("Yes");
                aGroupRight.setAllowEdit("Yes");
                aGroupRight.setAllowView("Yes");
                aGroupRight.setAllowDelete("Yes");
    ...
    /**
     * @param AllView the AllView to set
     */
    ...
    /**
     * @param AllAdd the AllAdd to set
     */
    public void setAllAdd(boolean AllAdd) {
        this.AllAdd = AllAdd;
    }
    ...
    /**
     * @param AllEdit the AllEdit to set
     */
    ...
    /**
     * @param AllDelete the AllDelete to set
     */
