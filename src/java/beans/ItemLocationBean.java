...
public class ItemLocationBean implements Serializable {
    ...
    public void reportItemLocations(int aStoreId, long aLocationId, Item aItem) {
        String sql = "{call sp_report_item_location(?,?,?)}";
        ResultSet rs = null;
        this.setItemLocationsList(new ArrayList<>());
        try (
                Connection conn = DBConnection.getMySQLConnection();
                PreparedStatement ps = conn.prepareStatement(sql);) {
            try {
    ...
    /**
     * @return the ItemLocationsList
     */
    public List<ItemLocation> getItemLocationsList() {
        return ItemLocationsList;
    }

    /**
     * @param ItemLocationsList the ItemLocationsList to set
     */
