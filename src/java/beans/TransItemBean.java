...
public class TransItemBean implements Serializable {
    ...
    public void adjustStockForTransItems(Trans t, List<TransItem> tis) {
        try {
            ...
            while (ListItemIndex < ListItemNo) {
                ...
                if (t.getTransactionTypeId() == 67 && tis.get(ListItemIndex).getBase_unit_qty() <= 0 && tis.get(ListItemIndex).getQty_damage() <= 0) {
                    //do nothing
                } else {
        ...
        try {
            ...
            while (ListItemIndex < ListItemNo) {
                ...
                if (aTransTypeId == 67 && ati.get(ListItemIndex).getItemQty() <= 0 && ati.get(ListItemIndex).getQty_damage() <= 0) {
                    ...
                    inserted = this.insertTransItemOut(aStoreId, aTransTypeId, aTransReasonId, aSaleType, aTrans, ati.get(ListItemIndex));
                }
            ...
            try (
                ...
                if (transitem.getTransactionItemId() == 0) {
                    ...
                    if ("UNPACK".equals(transtype.getTransactionTypeName())) {
                        ...
                        cs.setString("in_stock_effect", "C");
                        cs.executeUpdate();
                        InsertedId2 = cs.getLong("out_transaction_item_id");
                        try {
                            if (InsertedId2 > 0) {
                                ItemBean ib = new ItemBean();
                                Transaction_item_unit tiu2 = new Transaction_item_unit();
                                tiu2.setTransaction_item_id(InsertedId2);
                                tiu2.setUnit_id(transitem.getUnit_id2());
                                tiu2.setBase_unit_qty(transitem.getBase_unit_qty2());
                                new TransItemExtBean().insertTransaction_item_unit(tiu2);
                            }
                        } catch (Exception e) {
                            LOGGER.log(Level.ERROR, e);
                        }
                    ...
                    try {
                        ...
                        if (InsertedId2 > 0) {
                            if (aTransTypeId == 67 && transitem.getBase_unit_qty() <= 0 && transitem.getQty_damage() <= 0) {
                                ...
                                this.adjustStockForTransItem(new TransBean().getTrans(transitem.getTransactionId()), this.getTransItem(InsertedId2));
                            }
                        }
                    } catch (Exception e) {
                        //do nothing
    ...
    public void adjustStockForTransItem(Trans aTrans, TransItem aTransitem) {
        ...
        try {
            ...
            Store store2 = new StoreBean().getStore(aTrans.getStore2Id());
            //update stock
            if (("SALE INVOICE".equals(transtype.getTransactionTypeName()) && !new Parameter_listBean().getParameter_listByContextName("COMPANY_SETTING", "DEPLETE_SOLD_STOCK_UPON").getParameter_value().equals("0")) || ("GOODS DELIVERY".equals(transtype.getTransactionTypeName()) && !new Parameter_listBean().getParameter_listByContextName("COMPANY_SETTING", "DEPLETE_SOLD_STOCK_UPON").getParameter_value().equals("1"))) {
                //ingore stock update
            } else {
                if (new ItemBean().getItem(aTransitem.getItemId()).getIsTrack() == 1) {
                    if ("SALE INVOICE".equals(transtype.getTransactionTypeName()) || "DISPOSE STOCK".equals(transtype.getTransactionTypeName()) || ("STOCK ADJUSTMENT".equals(transtype.getTransactionTypeName()) && aTransitem.getNarration().equals("Subtract")) || "STOCK CONSUMPTION".equals(transtype.getTransactionTypeName()) || "GOODS DELIVERY".equals(transtype.getTransactionTypeName())) {
                        ...
                        i = new StockBean().subtractStock(stock, aTransitem.getBase_unit_qty());
                        ...
                        new Stock_ledgerBean().callInsertStock_ledger(TableName, "Subtract", stock, aTransitem.getItemQty(), "Add", aTrans.getTransactionTypeId(), aTrans.getTransactionId(), aTrans.getAddUserDetailId(), aTransitem.getTransactionItemId());
                    }
                    if ("HIRE INVOICE".equals(transtype.getTransactionTypeName())) {
                        ...
                        sout.setQty_out(aTransitem.getBase_unit_qty());
                    ...
                    if ("HIRE RETURN NOTE".equals(transtype.getTransactionTypeName())) {
                        ...
                        sout.setCode_specific(aTransitem.getCodeSpecific());
                        sout.setDesc_specific(aTransitem.getDescSpecific());
                        sout.setQty_out(aTransitem.getBase_unit_qty() + aTransitem.getQty_damage());
                        sout.setTransactor_id(aTrans.getTransactorId());
                        sout.setSite_id(aTrans.getSite_id());
                    ...
                    if ("ITEM RECEIVED".equals(transtype.getTransactionTypeName()) || (transtype.getTransactionTypeId() == 1 && transreason.getTransactionReasonId() == 29) || ("STOCK ADJUSTMENT".equals(transtype.getTransactionTypeName()) && aTransitem.getNarration().equals("Add")) || (transtype.getTransactionTypeId() == 1 && PurInvoMode == 1)) {
                        ...
                        if (StkBean.getStock(store.getStoreId(), aTransitem.getItemId(), aTransitem.getBatchno(), aTransitem.getCodeSpecific(), aTransitem.getDescSpecific()) != null) {
                            ...
                            stock.setUnitCost(UnitCostPrice);
                            i = new StockBean().addStock(stock, aTransitem.getBase_unit_qty());
                            stock.setSpecific_size(aTransitem.getSpecific_size());
                            String TableName = new Parameter_listBean().getParameter_listByContextName("COMPANY_SETTING", "CURRENT_TABLE_NAME_STOCK_LEDGER").getParameter_value();
                            new Stock_ledgerBean().callInsertStock_ledger(TableName, "Add", stock, aTransitem.getItemQty(), "Add", aTrans.getTransactionTypeId(), aTrans.getTransactionId(), aTrans.getAddUserDetailId(), aTransitem.getTransactionItemId());
                        } else {
                            //insert
                            Stock stock = new Stock();
                            int i = 0;
                            ...
                            stock.setCurrentqty(aTransitem.getBase_unit_qty());
                    ...
                    if ("UNPACK".equals(transtype.getTransactionTypeName()) && aTransitem.getStockEffect().equals("D")) {
                        Stock stock = new Stock();
                        int i = 0;
                        stock.setStoreId(store.getStoreId());
                        stock.setItemId(aTransitem.getItemId());
                    ...
                    if ("UNPACK".equals(transtype.getTransactionTypeName()) && aTransitem.getStockEffect().equals("C")) {
                        if (StkBean.getStock(store.getStoreId(), aTransitem.getItemId(), aTransitem.getBatchno(), aTransitem.getCodeSpecific(), aTransitem.getDescSpecific()) != null) {
                            //update/add
                            Stock stock = new Stock();
                            int i = 0;
                            ...
                            LatestTransItemUnitCostPrice = this.getItemLatestUnitCostPrice(aTransitem.getItemId(), stock.getBatchno(), stock.getCodeSpecific(), stock.getDescSpecific(), aTransitem.getUnit_id(), new AccCurrencyBean().getLocalCurrency().getCurrencyCode(), 1);
    ...
    public int insertTransItemOut(int aStoreId, int aTransTypeId, int aTransReasonId, String aSaleType, Trans aTrans, TransItem transitem) {
        int inserted = 0;
        String sql = null;
        String msg = "";
        sql = "{call sp_insert_transaction_item_out(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)}";
        try (
            ...
            if ("UNPACK".equals(transtype.getTransactionTypeName())) {
                ...
                cs.setString("in_stock_effect", "C");
                cs.executeUpdate();
                long InsertedId2 = cs.getLong("out_transaction_item_id");
                try {
                    if (InsertedId2 > 0) {
                        ItemBean ib = new ItemBean();
                        Transaction_item_unit tiu2 = new Transaction_item_unit();
                        ...
                        ib.setItemUnitList(iuList, ib.getItem(transitem.getItemId2()), 1);
                        tiu2.setUnit_id(iuList.get(0).getUnit_id());
                        tiu2.setBase_unit_qty(transitem.getItemQty2());
                        new TransItemExtBean().insertTransaction_item_unit(tiu2);
                    }
                } catch (Exception e) {
                    LOGGER.log(Level.ERROR, e);
    ...
    public void saveDraftTransItem(Trans aTrans, TransItem transitem) {
        ...
        if (1 == 2) {
            ...
            try (
                ...
                try {
                    if (TransItemHistId > 0 && transitem.getTransItemExciseObj().getExcise_duty_code().length() > 0) {
                        Transaction_item_hist_excise tihe = new Transaction_item_hist_excise();
                        new TransItemExtBean().setHistExciseFromTrans(transitem.getTransItemExciseObj(), tihe);
                        tihe.setTransaction_item_hist_id(TransItemHistId);
                        tihe.setTransaction_item_excise_id(0);
                        tihe.setTransaction_item_id(0);
                        new TransItemExtBean().insertTransaction_item_hist_excise(tihe);
                    }
                } catch (Exception e) {
                    LOGGER.log(Level.ERROR, e);
                }
        ...
        try (
            ...
            try {
                if (InsertedId1 > 0) {
                    Transaction_item_unit tiu = new Transaction_item_unit();
                    tiu.setTransaction_item_id(InsertedId1);
                    transitem.setUnit_id(new ItemBean().getItem(transitem.getItemId()).getUnitId());
                    tiu.setUnit_id(transitem.getUnit_id());
                    tiu.setBase_unit_qty(transitem.getItemQty());
                    new TransItemExtBean().insertTransaction_item_unit(tiu);
                }
            } catch (Exception e) {
                LOGGER.log(Level.ERROR, e);
            ...
            try {
                if (InsertedId2 > 0) {
                    ...
                    Transaction_item_unit tiu2 = new Transaction_item_unit();
                    tiu2.setTransaction_item_id(InsertedId2);
                    transitem.setUnit_id2(new ItemBean().getItem(transitem.getItemId2()).getUnitId());
                    tiu2.setUnit_id(transitem.getUnit_id2());
                    tiu2.setBase_unit_qty(transitem.getItemQty2());
                    new TransItemExtBean().insertTransaction_item_unit(tiu2);
                }
    ...
    public TransItem getTransItemFromResultSet(ResultSet aResultSet) {
        try {
            ...
            try {
                transitem.setUnit_id(aResultSet.getInt("unit_id"));
            } catch (Exception e) {
                transitem.setUnit_id(0);
            }
            try {
                transitem.setBase_unit_qty(aResultSet.getDouble("base_unit_qty"));
            } catch (Exception e) {
                transitem.setBase_unit_qty(0);
            }
            return transitem;
        } catch (Exception e) {
            LOGGER.log(Level.ERROR, e);
    ...
    public void setTransItemsFromHist(List<TransItem> aTransItems, long aTransactionHistId) {
        ...
        try (
            ...
            while (rs.next()) {
                ti = this.getTransItemFromResultSet(rs);
                this.updateLookUpsUI(ti);
                Transaction_item_excise tie = new TransItemExtBean().getTransaction_item_exciseByItemHistId(rs.getLong("transaction_item_hist_id"));
                if (null != tie) {
                    ti.setTransItemExciseObj(tie);
                }
                aTransItems.add(ti);
            }
        } catch (Exception e) {
    ...
    public List<TransItem> getTransItemsSummaryByItemTypeDrNote(long aTransactionId) {
        String sql;
        sql = "{call sp_search_trans_item_summary_by_item_type_dr_note(?)}";
        ResultSet rs = null;
        List<TransItem> transitems = new ArrayList<>();
        TransItem transitem = null;
        try (
    ...
    public List<TransItem> getInventoryCostByTransDrNote(long aTransactionId) {
        String sql;
        sql = "{call sp_search_inventory_cost_by_trans_dr_note(?)}";
        ResultSet rs = null;
        List<TransItem> transitems = new ArrayList<>();
        TransItem transitem = null;
        try (
    ...
    public List<TransItem> getInventoryItemTypeCostByTransDrNote(long aTransactionId) {
        String sql;
        sql = "{call sp_search_inventory_item_type_cost_by_trans_dr_note(?)}";
        ResultSet rs = null;
        List<TransItem> transitems = new ArrayList<>();
        TransItem transitem = null;
        try (
...
//                st = sb.getStock(new GeneralUserSetting().getCurrentStore().getStoreId(), NewTransItem.getItemId(), NewTransItem.getBatchno(), NewTransItem.getCodeSpecific(), NewTransItem.getDescSpecific());
    ...
    public String addTransItemCEC(int aStoreId, int aTransTypeId, int aTransReasonId, String aSaleType, Trans aTrans, List<TransItem> aActiveTransItems, TransItem NewTransItem, Item aSelectedItem) {
        ...
        try {
            ...
            if (aSelectedItem.getIsTrack() == 1 && "SALE INVOICE".equals(transtype.getTransactionTypeName()) & "Yes".equals(CompanySetting.getIsAllowAutoUnpack()) && new Parameter_listBean().getParameter_listByContextNameMemory("COMPANY_SETTING", "DEPLETE_SOLD_STOCK_UPON").getParameter_value().equals("0")) {
                ...
                //double TotalQtyFromListAndNew = TotalQtyFromList + NewTransItem.getItemQty();
                double TotalQtyFromListAndNew = TotalQtyFromList + NewTransItem.getBase_unit_qty();
                double SmallItemQtyNeeded = 0;
                try {
                    SmallItemQtyNeeded = TotalQtyFromListAndNew - st.getCurrentqty();
                } catch (NullPointerException npe) {
                ...
                } else if ((NewTransItem.getBase_unit_qty() + this.getListTotalItemBatchQty(aActiveTransItems, NewTransItem.getItemId(), NewTransItem.getBatchno(), NewTransItem.getCodeSpecific(), NewTransItem.getDescSpecific())) > st.getCurrentqty()) {
                    if (this.autoUnpackItem(NewTransItem, SmallItemQtyNeeded) == 1) {
                    } else {
                    }
                }
            ...
            if (aSelectedItem.getIsTrack() == 1 && null != st && !"HIRE RETURN INVOICE".equals(transtype.getTransactionTypeName())) {
                if (transtype.getTransactionTypeName().equals("STOCK ADJUSTMENT")) {
                    ...
                    if (NewTransItem.getNarration().equals("Subtract") && ((NewTransItem.getBase_unit_qty() + this.getListTotalItemBatchQty(aActiveTransItems, NewTransItem.getItemId(), NewTransItem.getBatchno(), NewTransItem.getCodeSpecific(), NewTransItem.getDescSpecific())) > st.getCurrentqty())) {
                        //check if supplied qty + existing qty is more than total current stock qty
                    ...
                    if (transtype.getTransactionTypeName().equals("SALE INVOICE") && !new Parameter_listBean().getParameter_listByContextNameMemory("COMPANY_SETTING", "DEPLETE_SOLD_STOCK_UPON").getParameter_value().equals("0")) {
                        ...
                        //if ((NewTransItem.getItemQty() + this.getListTotalItemBatchQty(aActiveTransItems, NewTransItem.getItemId(), NewTransItem.getBatchno(), NewTransItem.getCodeSpecific(), NewTransItem.getDescSpecific())) > st.getCurrentqty()) {
                        if ((NewTransItem.getBase_unit_qty() + this.getListTotalItemBatchQty(aActiveTransItems, NewTransItem.getItemId(), NewTransItem.getBatchno(), NewTransItem.getCodeSpecific(), NewTransItem.getDescSpecific())) > st.getCurrentqty()) {
                            //check if supplied qty + existing qty is more than total current stock qty
                            status = "Insufficient Stock";
                            StockFail2 = 1;
                        }
            ...
            if (StockFail1 == 0 && StockFail2 == 0 && StockFail3 == 0) {
                ...
                //Excise Duty - Start
                if (NewTransItem.getTransItemExciseObj() == null) {
                    NewTransItem.setTransItemExciseObj(new Transaction_item_excise());
                }
                if (NewTransItem.getTransItemExciseObj().getExcise_duty_code() == null) {
                    NewTransItem.getTransItemExciseObj().setExcise_duty_code("");
                }
    ...
    public void updateLookUpsUI(TransItem aTransItem) {
        try {
            ...
            if (null == aTransItem) {
                ...
                try {
                    if (aTransItem.getUnit_id() > 0) {
                        aTransItem.setUnitSymbol(new UnitBean().getUnit(aTransItem.getUnit_id()).getUnitSymbol());
                    } else {
    ...
    public void editTransItemCEC(int aTransTypeId, int aTransReasonId, String aSaleType, Trans aTrans, List<TransItem> aActiveTransItems, TransItem ti) {
        try {
            ...
            //Excise Duty - Start
            if (ti.getTransItemExciseObj() == null) {
                ti.setTransItemExciseObj(new Transaction_item_excise());
            }
            if (ti.getTransItemExciseObj().getExcise_duty_code() == null) {
                ti.getTransItemExciseObj().setExcise_duty_code("");
            }
    ...
    public void editTransItemCrDr(int aTransTypeId, int aTransReasonId, String aSaleType, Trans aTrans, List<TransItem> aActiveTransItems, TransItem ti) {
        try {
            TransactionType transtype = new TransactionTypeBean().getTransactionType(aTransTypeId);
            TransactionReason transreason = new TransactionReasonBean().getTransactionReason(aTransReasonId);
            this.updateBaseUnityQty(ti);
    ...
    public void editTransItemUponUnitChange(int aTransTypeId, int aTransReasonId, TransItem ti) {
        TransactionType transtype = new TransactionTypeBean().getTransactionType(aTransTypeId);
        TransactionReason transreason = new TransactionReasonBean().getTransactionReason(aTransReasonId);
        //first update unit prices
        double IncludedVat;
        ...
        try {
            ...
            try {
                if ("SALE INVOICE".equals(new GeneralUserSetting().getCurrentTransactionTypeName()) || "SALE ORDER".equals(new GeneralUserSetting().getCurrentTransactionTypeName())) {
                    ...
                    double UnitCost = new StockBean().getItemUnitCostPrice(new GeneralUserSetting().getCurrentStore().getStoreId(), NewTransItem.getItemId(), NewTransItem.getBatchno(), NewTransItem.getCodeSpecific(), NewTransItem.getDescSpecific());
                    int FrmUnitId = new ItemBean().getItem(ti.getItemId()).getUnitId();
                    int ToUnitId = ti.getUnit_id();
                    double UnitConversionRate = new ItemBean().getUnitConversionRate(NewTransItem.getItemId(), FrmUnitId, ToUnitId);
                    UnitCost = UnitCost * UnitConversionRate;
                    ti.setUnitCostPrice(UnitCost);
                    //CHANGED
                    double UnitCostExcVat = 0;
                    if (ti.getUnitVat() > 0) {
                        UnitCostExcVat = ti.getUnitCostPrice() / (1 + (ti.getVatPerc() / 100));
        ...
        try {
            if (aItem.getIsTrack() == 0) {
                ...
                if (st == null || st.getCurrentqty() == 0) {//this item and batch does not exist in this store OR its quatity is 0 (out of stock)
                    ...
                    ti.setItemQty(NewTransItem.getItemQty());
                    ti.setBase_unit_qty(NewTransItem.getItemQty());
                    ti.setUnitPrice(NewTransItem.getUnitPrice());
                    ti.setUnitTradeDiscount(NewTransItem.getUnitTradeDiscount());
                    ti.setAmount(NewTransItem.getAmount());
                    ti.setVatRated(NewTransItem.getVatRated());
                    ...
                    ti.setItemId2(NewTransItem.getItemId2());
                    ti.setItemQty2(NewTransItem.getItemQty2());
                    ti.setBase_unit_qty2(NewTransItem.getItemQty2());
                    try {
                        ti.setItemExpryDate(st.getItemExpDate());
                    } catch (Exception e) {
                        ti.setItemExpryDate(null);
                    ...
                    ti.setUnit_id(new ItemBean().getItem(ti.getItemId()).getUnitId());
                    ti.setUnit_id2(new ItemBean().getItem(ti.getItemId2()).getUnitId());
                    //for profit margin
                    ti.setUnitCostPrice(0);
                    ti.setUnitProfitMargin(0);
                ...
                if (itm.getIsTrack() == 1) {
                    ...
                    try {
                        if (st == null) {
                            ...
                            SmallItemQtyNeeded = aActiveTransItems.get(i).getBase_unit_qty();
                        } else {
                            SmallItemQtyNeeded = aActiveTransItems.get(i).getBase_unit_qty() - st.getCurrentqty();
                        }
                    } catch (NullPointerException npe) {
                    }
                    //Unpackaging if needed
    ...
    public int autoUnpackItem(TransItem aSmallTransItem, double aSmallItemQtyNeeded) {
        long BigItemQtyToUnpack = 0;
        try {
            //1. check if Small Item has big item
            ItemMap aItemMap;
            try {
                aItemMap = new ItemMapBean().getItemMapBySmallItemId(aSmallTransItem.getItemId());
                BigItemQtyToUnpack = Math.round(Math.ceil(aSmallItemQtyNeeded / aItemMap.getFractionQty()));
            } catch (NullPointerException npe) {
                BigItemQtyToUnpack = 0;
                aItemMap = null;
            }
            //2. check if needed Qty can be unpacked from the Big Item
            if (aItemMap != null & aSmallItemQtyNeeded > 0 & BigItemQtyToUnpack > 0) {
    ...
    public int itemExists(List<TransItem> aActiveTransItems, Long aItemId, String aBatchNo, String aCodeSpec, String aDescSpec, int aUnitId) {
        List<TransItem> ati = aActiveTransItems;
        int ItemFoundAtIndex = -1;
        int ListItemIndex = 0;
        int ListItemNo = ati.size();
        while (ListItemIndex < ListItemNo) {
            if (ati.get(ListItemIndex).getItemId() == aItemId && aBatchNo.equals(ati.get(ListItemIndex).getBatchno()) && aCodeSpec.equals(ati.get(ListItemIndex).getCodeSpecific()) && aDescSpec.equals(ati.get(ListItemIndex).getDescSpecific()) && ati.get(ListItemIndex).getUnit_id() == aUnitId) {
                ItemFoundAtIndex = ListItemIndex;
                break;
            } else {
                ItemFoundAtIndex = -1;
    ...
    public TransItem itemExistsObj(List<TransItem> aActiveTransItems, Long aItemId, String aBatchNo, String aCodeSpec, String aDescSpec, int aUnitId) {
        List<TransItem> ati = aActiveTransItems;
    ...
    public double getListTotalItemBatchQty(List<TransItem> aActiveTransItems, Long aItemI, String aBatchN, String aCodeSpec, String aDescSpec) {
        ...
        while (ListItemIndex < ListItemNo) {
            ...
            if (ati.get(ListItemIndex).getItemId() == aItemI && aBatchN.equals(BatcNo) && aCodeSpec.equals(CodeSp) && aDescSpec.equals(DescSp)) {
                ...
                TQty = TQty + ati.get(ListItemIndex).getBase_unit_qty();
            }
            ListItemIndex = ListItemIndex + 1;
        }
        return TQty;
    ...
    public String getAnyItemTotalQtyGreaterThanCurrentQty(List<TransItem> aActiveTransItems, int aStoreId, String aTransactionType) {
        ...
        while (ListItemIndex < ListItemNo && ((aTransactionType.equals("SALE INVOICE") && new Parameter_listBean().getParameter_listByContextNameMemory("COMPANY_SETTING", "DEPLETE_SOLD_STOCK_UPON").getParameter_value().equals("0")) || aTransactionType.equals("DEBIT NOTE") || aTransactionType.equals("TRANSFER") || aTransactionType.equals("DISPOSE STOCK") || aTransactionType.equals("STOCK ADJUSTMENT") || (aTransactionType.equals("GOODS DELIVERY") && new Parameter_listBean().getParameter_listByContextNameMemory("COMPANY_SETTING", "DEPLETE_SOLD_STOCK_UPON").getParameter_value().equals("1")))) {
            ...
            if (aTransactionType.equals("STOCK ADJUSTMENT") && ati.get(ListItemIndex).getNarration().equals("Add")) {
                ...
                if (this.isItemTotalQtyGreaterThanCurrentQty(aStoreId, ati.get(ListItemIndex).getItemId(), ati.get(ListItemIndex).getBatchno(), ati.get(ListItemIndex).getBase_unit_qty(), ati.get(ListItemIndex).getCodeSpecific(), ati.get(ListItemIndex).getDescSpecific())) {
    ...
    public void copyObjectTransItem(TransItem FromObj, TransItem ToObj) {
        if (ToObj != null) {
            ...
            ToObj.setUnit_id(FromObj.getUnit_id());
            ToObj.setBase_unit_qty(FromObj.getBase_unit_qty());
        }
    }
        ...
        try {
            ...
            if (aItem == null || transtype == null) {
                ...
                if (transtype.getTransactionTypeName().equals("ITEM RECEIVED") || transtype.getTransactionTypeName().equals("PRODUCTION") || transtype.getTransactionTypeName().equals("STOCK ADJUSTMENT") || transtype.getTransactionTypeName().equals("DISPOSE STOCK") || transtype.getTransactionTypeName().equals("PURCHASE INVOICE") || transtype.getTransactionTypeName().equals("STOCK CONSUMPTION")) {
                    ...
                    aTransItemToUpdate.setUnitCostPrice(this.getItemLatestUnitCostPrice(aItem.getItemId(), "", "", "", aTransItemToUpdate.getUnit_id(), CurCode, 1));
                }
    ...
    public double getItemLatestUnitCostPrice(long aItemId, String aBatchno, String aCodeSpec, String aDescSpec, int aToUnitId, String aToCurrencyCode, int aConvertCurrencyFlag) {
        ...
        if (LatestItemReceivedTransItemId > 0) {
            try {
                TransItem ti = new TransItemBean().getTransItem(LatestItemReceivedTransItemId);
                FrmUnitId = ti.getUnit_id();
    ...
    public void updateModelTransItemAutoAddCEC(int aStoreId, int aTransTypeId, int aTransReasonId, String aSaleType, Trans aTrans, TransItem aTransItemToUpdate, StatusBean aStatusBean, List<TransItem> aActiveTransItems, TransItem aSelectedTransItem, Item aSelectedItem, String aEntryMode) {//auto=1 for itemCode, auto=0 is for desc/code    ,2 is for other
        ...
        if (aSelectedItem == null) {
            ...
            aSelectedTransItem.setItemCode(aSelectedItem.getItemCode());
            aSelectedTransItem.setItemQty(1);
            double BaseQty = new ItemBean().getBaseUnitQty(aSelectedTransItem.getItemId(), aSelectedTransItem.getUnit_id(), aSelectedTransItem.getItemQty());
            if (BaseQty > 0) {
                aSelectedTransItem.setBase_unit_qty(BaseQty);
            } else {
                aSelectedTransItem.setUnit_id(aSelectedItem.getUnitId());
                aSelectedTransItem.setBase_unit_qty(aSelectedTransItem.getItemQty());
            }
            ...
            if (batches.size() > 1) {
                ...
                if (aSelectedItem.getIsTrack() == 1 && ("SALE INVOICE".equals(transtype.getTransactionTypeName()) || "SALE ORDER".equals(transtype.getTransactionTypeName())) & "Yes".equals(CompanySetting.getIsAllowAutoUnpack()) && new Parameter_listBean().getParameter_listByContextName("COMPANY_SETTING", "DEPLETE_SOLD_STOCK_UPON").getParameter_value().equals("0")) {
                    double TotalQtyFromList = 0;
                    try {
                    ...
                    double TotalQtyFromListAndNew = TotalQtyFromList + aSelectedTransItem.getBase_unit_qty();
                    try {
                        SmallItemQtyNeeded = TotalQtyFromListAndNew - st.getCurrentqty();
                    } catch (NullPointerException npe) {
                        SmallItemQtyNeeded = TotalQtyFromListAndNew;
    ...
    public void calUnpackedQty(TransItem aTransItem) {
        try {
            ...
            aTransItem.setBase_unit_qty2(aTransItem.getItemQty2());
        } catch (Exception e) {
            aTransItem.setItemQty2(0);
        }
    }
    ...
    public List<TransItem> getTransItemListCurLessPrevQty(List<TransItem> aCurTransItemList, Trans aCurTrans) {
        ...
        double PrevQty = 0;
        double CurQty = 0;
        double PrevQtyBaseUnit = 0;
        double CurQtyBaseUnit = 0;
        ...
        while (ListItemIndexCur < ListItemNoCur) {
            ...
            CurQtyBaseUnit = CurTransItem.getBase_unit_qty();
            ListItemIndexPrev = 0;
            while (ListItemIndexPrev < ListItemNoPrev) {
                ...
                if (CurTransItem.getItemId() == PrevTransItem.getItemId() && CurTransItem.getUnit_id() == PrevTransItem.getUnit_id() && ub.getEmptyIfNull(CurTransItem.getBatchno()).equals(ub.getEmptyIfNull(PrevTransItem.getBatchno())) && ub.getEmptyIfNull(CurTransItem.getCodeSpecific()).equals(PrevTransItem.getCodeSpecific()) && ub.getEmptyIfNull(CurTransItem.getDescSpecific()).equals(ub.getEmptyIfNull(PrevTransItem.getDescSpecific()))) {
                    PrevQty = PrevTransItem.getItemQty();
                    PrevQtyBaseUnit = PrevTransItem.getBase_unit_qty();
                    break;
                }
            ...
            if ((CurQty - PrevQty) != 0) {
                ...
                CurTransItem.setBase_unit_qty(CurQtyBaseUnit - PrevQtyBaseUnit);
