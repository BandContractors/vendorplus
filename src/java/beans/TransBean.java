...
import entities.PointsTransaction;
...
public class TransBean implements Serializable {
    ...
    public void saveTrans(Trans trans, List<TransItem> aActiveTransItems, Transactor aSelectedTransactor, Transactor aSelectedBillTransactor, UserDetail aTransUserDetail, Transactor aSelectedSchemeTransactor, UserDetail aAuthorisedByUserDetail, AccCoa aSelectedAccCoa) {
        String sql = null;
        String sql2 = null;
        String msg = "";
    ...
    public void updateTransV2(Trans aNewTrans, List<TransItem> aNewTransItems, Pay aPay) {
        ...
        if (grb.IsUserGroupsFunctionAccessAllowed(aCurrentUserDetail, aCurrentGroupRights, Integer.toString(aNewTrans.getTransactionReasonId()), "Edit") == 0) {
            ...
            //insert PointsTransaction for both the awarded and spent points to the stage area
            if (null != newPay && isTransCopySuccess && isTransItemCopySuccess && isTransItemReverseSuccess && isTransUpdateSuccess && tt.getTransactionTypeName().equals("SALE INVOICE")) {
                PointsCard pc = new PointsCard();
                pc = new PointsCardBean().getPointsCardByCardNumber(aNewTrans.getCardNumber());
                if ("SALE INVOICE".equals(tt.getTransactionTypeName()) && null != pc) {
                    //1. insert PointsTransaction for both the awarded and spent points to the stage area
                    PointsTransaction NewPointsTransaction = new PointsTransaction();
    ...
    public void updateTransCEC(String aLevel, int aStoreId, int aTransTypeId, int aTransReasonId, String aSaleType, Trans aNewTrans, List<TransItem> aNewTransItems, Pay aPay) {
        ...
        if (ValidationMessage.length() > 0) {
            ...
            //update for the points card transcation
            //insert PointsTransaction for both the awarded and spent points to the stage area
            if (null != newPay && isTransCopySuccess && isTransItemCopySuccess && isTransItemReverseSuccess && isTransUpdateSuccess && transtype.getTransactionTypeName().equals("SALE INVOICE") || "HIRE INVOICE".equals(transtype.getTransactionTypeName())) {
                PointsCard pc = new PointsCard();
                pc = new PointsCardBean().getPointsCardByCardNumber(aNewTrans.getCardNumber());
                if (("SALE INVOICE".equals(transtype.getTransactionTypeName()) || "HIRE INVOICE".equals(transtype.getTransactionTypeName())) && null != pc) {
                    //1. insert PointsTransaction for both the awarded and spent points to the stage area
                    PointsTransaction NewPointsTransaction = new PointsTransaction();
                    NewPointsTransaction.setPointsCardId(pc.getPointsCardId());
                    NewPointsTransaction.setTransactionDate(new java.sql.Date(aNewTrans.getTransactionDate().getTime()));
...
//                }
    ...
    public double getTotalVatCEC(Trans aTrans, List<TransItem> aActiveTransItems) {
        ...
        double VatPerc = CompanySetting.getVatPerc();
        if (aTrans.getCashDiscount() > 0 && TVat > 0 && VatPerc > 0) {
...
//    }
    ...
    public void reportSalesInvoiceDetail(Trans aTrans, TransBean aTransBean) {
        ...
        if (aTransBean.getFieldName().length() > 0) {
            ...
            sqlsum = "SELECT currency_code,sum(grand_total) as grand_total,sum(total_profit_margin) as total_profit_margin,sum(total_vat) as total_vat,sum(cash_discount) as cash_discount FROM transaction WHERE transaction_type_id IN(2,65,68)";
        }
        String wheresql = "";
        String ordersql = "";
        String ordersqlsum = "";
...
//        }
    ...
    public void reportTransItemDetail(Trans aTrans, TransBean aTransBean, Item aItem, Transactor aTransactor) {
        ...
        //String sqlsum = "";
//        if (aTransBean.getFieldName().length() > 0) {
//            sqlsum = "SELECT " + aTransBean.getFieldName() + ",currency_code,sum(grand_total) as grand_total,sum(total_profit_margin) as total_profit_margin,sum(total_vat) as total_vat,sum(cash_discount) as cash_discount FROM transaction WHERE transaction_type_id=2";
//        } else {
//            sqlsum = "SELECT currency_code,sum(grand_total) as grand_total,sum(total_profit_margin) as total_profit_margin,sum(total_vat) as total_vat,sum(cash_discount) as cash_discount FROM transaction WHERE transaction_type_id=2";
//        }
        String wheresql = "";
        String ordersql = "";
        //String ordersqlsum = "";
