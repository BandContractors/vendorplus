...
CREATE TABLE aa_over_paid_pay_trans as 
select * from pay_trans where trans_paid_amount>grand_total order by transaction_id asc;

CREATE TABLE aa_over_paid_pay as 
select * from pay where pay_id in (select distinct pay_id from aa_over_paid_pay_trans) order by pay_id asc;

update pay_trans PT SET PT.trans_paid_amount=PT.grand_total 
where PT.pay_trans_id>0 and PT.trans_paid_amount>PT.grand_total;
...
select transaction_id,sum(trans_paid_amount) as trans_paid_amount from pay_trans group by transaction_id
);
...
DROP TABLE aa_over_paid_pay_trans;
DROP TABLE aa_over_paid_pay;
