-- delete tables they exist
drop table transactor_fix;
drop table trans_total_paid;
drop table trans_sales_paid_bal;
drop table trans_purchase_paid_bal;

create table transactor_fix as
select 
tr.transactor_id,
tr.transactor_names,
(select sum(ifnull(grand_total,0)) from transaction t1 where t1.transaction_type_id IN(2,1) and t1.transactor_id=tr.transactor_id) as TTotal,
(select sum(ifnull(p.paid_amount,0)) from pay p where p.bill_transactor_id=tr.transactor_id and p.pay_type_id IN(14,15)) as TPaid,
...
create table trans_purchase_paid_bal as 
select t.transactor_id,t.transaction_id,t.grand_total,ifnull(tp.total_paid,0) as total_paid,
(t.grand_total-ifnull(tp.total_paid,0)) as tbal 
from transaction t 
inner join transactor_fix tf on t.transactor_id=tf.transactor_id 
...
delete from acc_journal where acc_journal_id>0 and account_code='2-00-000-010' and bill_transactor_id 
IN(select tx.transactor_id from transactor_fix tx);

SELECT max(job_id)+1 FROM acc_journal; -- X
...
sb.tbal,0,'CASH RECEIVABLE',(select ap.acc_period_id from acc_period ap where t.transaction_date between ap.start_date and ap.end_date),
NULL,1,0,Now(),1,NULL,NULL,X 
from trans_sales_paid_bal sb inner join transaction t on t.transaction_id=sb.transaction_id where sb.tbal>0;

SELECT max(job_id)+1 FROM acc_journal; -- Y
...
SELECT t.transaction_date,t.transaction_id,t.transaction_type_id,t.transaction_reason_id,
NULL,NULL,NULL,
t.store_id,t.transactor_id,'',46,'2-00-000-010','UGX',1,
0,sb.tbal,'CASH PAYABLE',(select ap.acc_period_id from acc_period ap where t.transaction_date between ap.start_date and ap.end_date),
NULL,1,0,Now(),1,NULL,NULL,Y 
from trans_purchase_paid_bal sb inner join transaction t on t.transaction_id=sb.transaction_id where sb.tbal>0;
